...
"""
DOGE DROP AUTO BOT - FIXED + CONFIG FILE
=========================================
- Saves init_data to config.json so you don't re-enter every run.
- Auto-reauth if ad_watch fails with "unknown error" (likely session expired).
- Retry mechanism for ad_watch with exponential backoff.
- Better logging and debugging.
- Displays user ID from init_data.

If ad_watch still fails, get a fresh init_data from @DogeFuacet_bot.
"""
...
FAUCETPAY_EMAIL = "tg6894031790@dogefaucet.app"  # CHANGE TO YOUR EMAIL
CAPTCHA_POSITION = 41  # try 41, 54, 61, 71, 81 if it fails
CONFIG_FILE = "config.json"  # saves init_data and user info
...
def print_banner():
    ...
    print(f"{G}========================================================{RESET}")
    print()
    print(f"{C}  [+] Mode      : {W}Auto Farm + Faucet Claim")
...
def print_session(user_data):
    ...
    print(f"{C}  User ID   : {W}{user_data.get('id', 'N/A')}")
...
class DogeDropBot:
    ...
    def load_config(self):
        """Load saved init_data from config file if exists."""
        if os.path.exists(CONFIG_FILE):
            try:
                with open(CONFIG_FILE, 'r') as f:
                    config = json.load(f)
                    saved = config.get('init_data')
                    if saved:
                        print(f"{C}📂 Loaded init_data from {CONFIG_FILE}{RESET}")
                        return saved
            except Exception as e:
                print(f"{Y}⚠️ Could not read config: {e}{RESET}")
        return None

    def save_config(self, init_data, user_id=None):
        """Save init_data and optionally user_id to config file."""
        config = {}
        if os.path.exists(CONFIG_FILE):
            try:
                with open(CONFIG_FILE, 'r') as f:
        ...
        try:
            ...
            print(f"{G}✅ Config saved to {CONFIG_FILE}{RESET}")
        except Exception as e:
            print(f"{R}✗ Could not save config: {e}{RESET}")

    def get_init_data(self):
        """Get init_data from config or prompt user."""
        print_banner()
        saved = self.load_config()
        if saved:
            # Verify it's valid by parsing user
            user_info = self.parse_init_data(saved)
            if user_info and user_info.get('id'):
                self.init_data = saved
                print(f"{G}✅ Loaded user ID: {user_info['id']}{RESET}")
                return
            else:
                print(f"{Y}⚠️ Saved init_data seems invalid, please enter a new one.{RESET}")

        print_setup()
        print(f"{Y}[!] Masukkan init_data (copy dari URL / Network tab).{RESET}")
        print(f"{D}  Contoh: user=%7B%22id%22%3A... atau query_id=AAG...{RESET}\n")
        print(f"{R}❗ Pastikan init_data masih valid (belum expired). Jika dapat hash_mismatch, ambil yang baru.{RESET}\n")
        while True:
            ...
            if raw:
                ...
                if user_info and user_info.get('id'):
                    self.init_data = raw
                    self.save_config(raw, user_info['id'])
    ...
    def authenticate(self):
        ...
        resp = self._post(f"{self.BASE_URL}/api/apps/{self.APP_ID}/functions/faucetAccount", payload)
        ...
        if resp.get("error") == "auth_failed" and "hash_mismatch" in str(resp):
            print(f"{R}✗ Hash mismatch – init_data sudah expired atau tidak valid.{RESET}")
            print(f"{Y}💡 Ambil init_data baru dari bot dan coba lagi.{RESET}")
        else:
    ...
    def fetch_settings(self):
        ...
        resp = self._get(f"{self.BASE_URL}/api/apps/{self.APP_ID}/entities/FaucetSettings", params=params)
    ...
    def watch_ad(self, ad_type, retries=3):
        """Watch an ad with retries and re-authentication if needed."""
        ...
        for attempt in range(retries):
            print(f"{M}▶ Attempt {attempt+1}/{retries}: Watching {ad_type.upper()} ad...{RESET}")
            ...
            resp = self._post(f"{self.BASE_URL}/api/apps/{self.APP_ID}/functions/creditReward", payload)

            # Debug: print full response if error
            if not resp.get("ok"):
                print(f"  {R}✗ Response: {json.dumps(resp, indent=2)}{RESET}")
                # Check if it's a session/expiry issue
                error_msg = resp.get("message", "") or resp.get("error", "")
                if "unknown error" in error_msg.lower() or "hash" in error_msg.lower() or "auth" in error_msg.lower():
                    print(f"{Y}⚠️ Possibly session expired, re-authenticating...{RESET}")
                    if self.authenticate():
                        # Update init_data if auth succeeded (maybe it changed?)
                        # But we still have same init_data, so maybe re-auth fixed something?
                        continue
                    else:
                        print(f"{R}✗ Re-auth failed, cannot proceed.{RESET}")
                        return False, 0
                else:
                    # Other error, maybe server side
                    if attempt < retries - 1:
                        sleep_time = (2 ** attempt) + random.uniform(0, 1)
                        print(f"{Y}⏱ Retrying in {sleep_time:.1f}s...{RESET}")
    ...
    def start_faucet_claim_session(self):
        ...
        if resp.get("ok"):
            ...
            if token:
                ...
                print(f"{Y}⚠️ No token in response, continuing without explicit token...{RESET}")
                return True
        else:
    ...
    def claim_faucet(self):
        ...
        create_resp = self._post(f"{self.BASE_URL}/api/apps/{self.APP_ID}/functions/faucetCaptcha", create_payload)
        ...
        verify_resp = self._post(f"{self.BASE_URL}/api/apps/{self.APP_ID}/functions/faucetCaptcha", verify_payload)
    ...
    def run(self):
        ...
        if not self.authenticate():
            print(f"{Y}💡 Jika terjadi hash_mismatch, hapus {CONFIG_FILE} dan jalankan ulang untuk input baru.{RESET}")
            input(f"{C}Press Enter to exit...{RESET}")
            return

        self.fetch_settings()

        # Claim faucet ONCE at the start
        self.claim_faucet()

        print(f"\n{Y}[!] Starting ad farming...{RESET}")
        ...
        while True:
            ...
            # Watch SHORT ads if available
            if self.short_ads_today < self.max_short:
                success, _ = self.watch_ad("short")
                if success:
                    self.short_ads_today += 1
                    continue

            # Watch LONG ads if available
            if self.long_ads_today < self.max_long:
                success, _ = self.watch_ad("long")
                if success:
                    self.long_ads_today += 1
                    continue

            # Watch TOWER ads if available
            if self.tower_ads_today < self.max_tower:
                success, _ = self.watch_ad("tower")
                if success:
                    self.tower_ads_today += 1
                    continue

            # If none succeeded, break
            break
