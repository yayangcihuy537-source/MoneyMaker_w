...
║   ╚═╝     ╚═╝╚═╝╚═╝  ╚═══╝╚═╝ ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝     ╚═╝╚══════╝║
║                                                             ║
║         🚀 MINIGRAMX X LITOSHIPAY - PARALLEL BOT           ║
...
# ========== Base Bot ==========
class BaseBot:
    def __init__(self, url, cfg, game_list, game_map, name, currency="GRAM"):
        self.url = url
        self.cfg = cfg
    ...
    def get_dashboard(self):
        ...
        if resp and resp.status_code == 200:
            html = resp.text
            patterns = [
                r'id="header-user-balance"[^>]*>([\d.,]+\s*GRAM|[\d.,]+)',
...
# ========== SETUP ==========
def setup_bots():
    print(f"{Y}📝 Setup MiniGramX dan LitoshiPay{RESET}")
    
    init1 = input(f"{C}Masukkan init_data untuk MiniGramX: {W}").strip()
    if init1:
        cfg1 = BaseConfig(MINI_CONFIG)
        ...
        try:
            ...
            pass
        cfg1.save()
        print(f"{G}✅ MiniGramX tersimpan{RESET}")
    else:
        print(f"{R}❌ MiniGramX kosong, dilewati{RESET}")
    
    init2 = input(f"{C}Masukkan init_data untuk LitoshiPay: {W}").strip()
    if init2:
        cfg2 = BaseConfig(LITOSHI_CONFIG)
        ...
        try:
            ...
            pass
        cfg2.save()
        print(f"{G}✅ LitoshiPay tersimpan{RESET}")
    else:
        print(f"{R}❌ LitoshiPay kosong, dilewati{RESET}")
...
def main():
    while True:
        ...
        print(f"""
{PURPLE}╔══════════════════════════════════════════════════════════╗
║   {GOLD}🚀 MINIGRAMX X LITOSHIPAY - PARALLEL BOT          {PURPLE}║
...
""")
        ...
        if choice == '0':
            ...
            if not mcfg.load() or not mcfg.init_data:
                print(f"{R}❌ MiniGramX init_data belum disetup (menu 2){RESET}")
                input("Enter...")
                continue
            if not lcfg.load() or not lcfg.init_data:
                print(f"{R}❌ LitoshiPay init_data belum disetup (menu 2){RESET}")
                input("Enter...")
                continue
            mbot = MiniBot(mcfg)
            lbot = LitoshiBot(lcfg)
            try:
                parallel_pair([mbot, lbot])
