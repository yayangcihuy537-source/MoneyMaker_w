...
function banner() {
    echo cyan . "
╔══════════════════════════════════════════════════════════════╗
║                  OURCOINCASH AUTO PTC + DAILY CLAIM         ║
╠══════════════════════════════════════════════════════════════╣
║  👨‍💻 Developer : " . putih . "ScriptyXSouu" . cyan . "                               ║
║  🌐 Website   : " . putih . "ourcoincash.xyz" . cyan . "                             ║
║  🐘 Language  : " . putih . "PHP" . cyan . "                                         ║
║  ⚡ Version   : " . putih . "version 1.1" . cyan . "                                         ║
...
function claim_daily($coki, $ua) {
    ...
    // Buka halaman bonus dulu
    $ref_dashboard = $headers_get;
    $ref_dashboard[] = "Referer: " . host . "/dashboard";
    $bonus_page = requests(host . "/bonus", "GET", [], $ref_dashboard);
    
    // Klaim bonus
    $ref_bonus = $headers_get;
    $ref_bonus[] = "Referer: " . host . "/bonus";
    $claim_res = requests(host . "/bonus/send?claim=", "GET", [], $ref_bonus);
    
    if (preg_match('/<div class="alert alert-success">(.*?)<\/div>/s', $claim_res, $m)) {
        return ["success" => strip_tags($m[1])];
    } elseif (preg_match('/<div class="alert alert-danger">(.*?)<\/div>/s', $claim_res, $m)) {
        return ["failed" => strip_tags($m[1])];
    }
    return false;
}
...
echo putih . "  " . LIME . "[2]" . putih . " 🎁 Daily Claim            " . "\n";
echo putih . "  " . TEAL . "[3]" . putih . " 🔄 Cek Balance            " . "\n";
echo putih . "  " . ORANGE . "[4]" . putih . " 🔄 Reset Config           " . "\n";
...
if ($pilih == '2') {
    echo putih . "🎁 Mencoba klaim bonus harian...\n" . reset;
    $daily_result = claim_daily($coki, $ua);
    if (is_array($daily_result) && isset($daily_result["success"])) {
        echo hijau . "✅ " . $daily_result["success"] . "\n" . reset;
    } elseif (is_array($daily_result) && isset($daily_result["failed"])) {
        echo merah . "❌ " . $daily_result["failed"] . "\n" . reset;
    } else {
        echo kuning . "⚠️ Gagal klaim bonus atau sudah diklaim.\n" . reset;
    }
