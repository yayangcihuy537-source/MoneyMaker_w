...
INIT_DATA = ""
AUTH_TOKEN = ""
APIKEY = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJzdXBhYmFzZSIsImlhdCI6MTc4NTc3MjY4MCwiZXhwIjo0OTQxNDQ2MjgwLCJyb2xlIjoiYW5vbiJ9.X3wvaxDhs_ObrxK1UOPk57lyBKIh4jVEDVxfo3P98UU"
START_PARAM = ""
...
# ═══════════════════════════════════════════════════════════════
# NETWORK LIST — MangoCash (10 network, sesuai foto)
# ═══════════════════════════════════════════════════════════════
NETWORKS = [
    "adsgram",
    "monetag",
...
def set_data(force=False):
    ...
    if not auth_token:
        auth_token = "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJzdXBhYmFzZSIsImlhdCI6MTc4NTc3MjY4MCwiZXhwIjo0OTQxNDQ2MjgwLCJyb2xlIjoiYW5vbiJ9.X3wvaxDhs_ObrxK1UOPk57lyBKIh4jVEDVxfo3P98UU"
...
# ============================================================
# FARMING — LOOP SAMPAI SEMUA NETWORK ABIS
# ============================================================
def start_farming():
    global HEADERS
                ...
                if not ready_networks:
                    if (len(cycle_success) + len(cycle_failed) + len(exhausted_networks)) >= len(NETWORKS):
                        ...
                        if still_available:
                            ...
                            print(f"{Y}🔄 Reset cycle, tunggu 60s...{RS}")
                    ...
                    for attempt in range(1, MAX_RETRY + 2):
                        ...
                        try:
                            ...
                            if ("ad_required" in err_str or
                                ...
                                print(f"  {Y}⛔ {net}: ABIS / limit server → EXHAUSTED{RS}")
                            ...
                            if attempt <= MAX_RETRY:
                                ...
                                print(f"  {R}⚠️ {net} gagal {MAX_RETRY+1}x → SKIP cycle ini{RS}")
                                cycle_failed.add(net)
                                cycle_done.add(net)
...
def check_balance():
    ...
    try:
        ...
        if not u:
            ...
            print(f"{Y}───────────────────────────{RS}")
            print(f"{G}🥭 Balance Cloud: {u.get('balance_cloud',0)}{RS}")
            print(f"{G}💰 Balance USDT : {u.get('balance_usdt',0)}{RS}")
