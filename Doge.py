...
# -*- coding: utf-8 -*-
"""
🐕 DOGE GENERATOR v3.0 — Dogecoin Auto Miner
...
class DogeMiner:
    ...
    def display_banner(self):
        balance_str = f"{self.num(self.balance, 8)} DOGE"
    ...
    def task_generator(self):
        ...
        if result and result.get("status") == "success":
            ...
            self.success += 1
            print(f"{G}✔ Generator : Claim +{self.num(reward, 8)} DOGE (Bonus: {self.num(bonus, 8)}){X}")
            return True
        else:
        ...
        if not last_daily or last_daily.split()[0] != today:
            ...
            if proof_token:
                ...
                if result and result.get("status") == "success":
                    ...
                    self.success += 1
                    print(f"{G}✔ Task : Daily Bonus +{self.num(amount, 8)} DOGE{X}")
                    claimed = True
        else:
            ...
            if last_double_activity and last_double_activity.split()[0] == today:
                ...
                if proof_token:
                    ...
                    if result and result.get("status") == "success":
                        ...
                        self.counter["tasks"] += 1
                        self.success += 1
                        print(f"{G}✔ Task : Double Reward +{self.num(amount, 8)} DOGE{X}")
                        claimed = True
    ...
    def task_games(self):
        ...
        while True:
            ...
            if not safe_tiles or can_cashout:
                print(f"{C}[GAME] Cashing out {self.num(current_earnings, 8)} DOGE...{X}")
                ...
                if result and result.get("status") == "success":
                    ...
                    print(f"{G}✔ Game : Cashout +{self.num(earned_amount, 8)} DOGE{X}")
                    continue
                else:
                    error_msg = result.get("message", "Unknown error") if result else "No response"
        ...
        if games_played > 0:
            print(f"{G}✔ Game : Completed {games_played} game(s), earned {self.num(total_earned_this_session, 8)} DOGE{X}")
    ...
    def run_miner(self):
        ...
        try:
            ...
            print(f"\n\n{G}[STOPPED] Doge Generator stopped{X}")
            self.display_session_summary()
    ...
    def main_menu(self):
        while True:
            ...
            if choice == "1":
                ...
                print(f"{G}[EXIT] Thank you for using Doge Generator!{X}")
                sys.exit(0)
...
if __name__ == "__main__":
    ...
    print(f"{G}🚀 Menjalankan Doge Generator...{W}\n")
