...
"""
╔═══════════════════════════════════════════════════════════════════╗
║  🐕 SHIBA INU TAP BOT v1.1 — Auto Retry + Rate Limit Handler    ║
║  🔐 Login via InitData                                         ║
...
║  {WHITE}╚══════╝╚═╝  ╚═╝╚═╝╚═════╝ ╚═╝  ╚═╝    ╚═╝╚═╝  ╚═══╝ ╚═════╝ {CYAN}║
║                                                                   ║
║            {GREEN}🐕 SHIBA INU TAP AUTO BOT v1.1 🐕{CYAN}               ║
║                                                                   ║
...
# ==================== KONFIGURASI ====================
CONFIG_FILE = "shiba_config.json"
BASE_URL = "https://panel-api.bleon.net"
BOT_NAME = "SHIBAInuTapbot"
...
class ShibaTapBot:
    ...
    def tap(self, taps: int = 20) -> bool:
        ...
        if result and result.get("ok"):
            ...
            if 'throttle' in str(error).lower() or 'too many' in str(error).lower():
                ...
                # setelah cooldown, kita bisa retry dengan memanggil tap lagi, tapi biarkan caller yang handle
                return False
    ...
    def show_status(self):
        ...
        print(f"""
{GREEN}╔══════════════════════════════════════════════════════════╗
║  🐕 SHIBA INU TAP STATUS                                  ║
╠══════════════════════════════════════════════════════════╣
║  {WHITE}Balance {GREEN}: {self.balance} SHIB
║  {WHITE}Energy  {GREEN}: {self.energy}
║  {WHITE}Taps   {GREEN}: {self.tap_count}
║  {WHITE}Streak {GREEN}: {self.streak_count}
║  {WHITE}Earned {GREEN}: {self.total_earned} SHIB
...
""")
        ...
        if choice == '0':
            ...
            if not config.init_data:
                ...
                continue
            bot = ShibaTapBot(config.init_data, config.tap_limit, config.withdraw_address,
