...
SESSION_FILE = "telegram_session_ltc"
...
class GhostMiner:
    ...
    def dashboard(self):
        ...
        print(f"\n{C}╔══════════════════════════════════════════════════════════════╗")
        print(f"║ {M}SCRIPT      {W}» {C}Litecoin Generator{' ' * 35}║")
        print(f"║ {M}BALANCE     {W}» {G}{self.num(self.balance, 8)} LTC{' ' * (38 - len(self.num(self.balance, 8)))}║")
    ...
    def task_generator(self):
        ...
        if result and result.get("status") == "success":
            ...
            print(f"{G}[SUCCESS] Claimed {self.num(reward, 8)} LTC (Bonus: {self.num(bonus, 8)}){X}")
        ...
        if not last_daily or last_daily.split()[0] != today:
            ...
            if proof_token:
                ...
                if result and result.get("status") == "success":
                    ...
                    print(f"{G}[SUCCESS] Claimed {self.num(amount, 8)} LTC{X}")
                    claimed = True
        else:
    ...
    def task_games(self):
        ...
        while True:
            ...
            if has_active and active_game:
                print(f" {M}CURRENT {W}» {G}{self.num(current_earnings, 8)} LTC{X}")
            print(f" {M}MODE    {W}» {C}{self.selected_difficulty.upper()}{X}")
            print(f" {M}GAMES   {W}» {C}{games_played}{X}")
            print(f"{C}{'=' * 54}{X}")

            if lives <= 0 and not has_active:
                print(f"{Y}[ENERGY] No energy left! Used {games_played} game(s), earned {self.num(total_earned_this_session, 8)} LTC{X}")
                break
            ...
            if not safe_tiles or can_cashout:
                print(f"{C}[CASHOUT] Cashing out {self.num(current_earnings, 8)} LTC...{X}")
                ...
                if result and result.get("status") == "success":
                    ...
                    print(f"{G}[CASHOUT] Earned {self.num(earned_amount, 8)} LTC{X}")
            ...
            if safe_tiles:
                ...
                if open_res and open_res.get("status") == "success":
                    ...
                    if tile_result == "bomb":
                        print(f"  {R}💣 Tile {tile_to_open:02} » BOMB! (Earned: {self.num(earned, 8)} LTC){X}")
        ...
        if games_played > 0:
            print(f"{G}[GAMES] Completed {games_played} game(s), earned {self.num(total_earned_this_session, 8)} LTC{X}")
    ...
    def run_miner(self):
        ...
        try:
            ...
            print(f"\n\n{G}[STOPPED] Litecoin Generator stopped{X}")
            print(f"\n{C}{'=' * 54}{X}")
    ...
    def main_menu(self):
        while True:
            ...
            if choice == "1":
                ...
                print(f"{G}[EXIT] Thank you for using Litecoin Generator!{X}")
                sys.exit(0)
...
if __name__ == "__main__":
    ...
    print(f"{G}🚀 Menjalankan Litecoin Generator...{W}\n")
